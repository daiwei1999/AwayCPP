...
void initEngine(struct engine* engine)
{
	...
	engine->m_cameraController = new away::HoverController(engine->m_view->getCamera(), 45, 20, 600);
...
void initScene(struct engine* engine)
{
	...
	readFileToByteArray(engine, "snow.atf", textureBytes);
	away::ATFCubeTexture* texture = new away::ATFCubeTexture(textureBytes);

	away::ColorMaterial* material = new away::ColorMaterial(0xFFFFFF);
	material->setSpecular(0.5f);
	material->setAmbientColor(0x111199);
	material->addMethod(new away::EnvMapMethod(texture, 1));

	engine->m_scene->addChild(new away::Mesh(new away::TorusGeometry(150, 60, 40, 20), material));
	engine->m_scene->addChild(new away::SkyBox(texture));
}
